...
RUN tar xzvf askar.tar.gz && cd aries-askar-${ASKAR_VERSION} && cargo build
...
RUN tar xzvf indy-credx.tar.gz && cd indy-shared-rs-${INDY_CREDX_VERSION} && cargo build
...
COPY --from=askar /askar/aries-askar-${ASKAR_VERSION}/target/debug/libaries_askar.so /usr/local/lib/python3.10/site-packages/aries_askar/libaries_askar.so
COPY --from=indy-credx /indy/indy-shared-rs-${INDY_CREDX_VERSION}/target/debug/libindy_credx.so /usr/local/lib/python3.10/site-packages/indy_credx/libindy_credx.so
